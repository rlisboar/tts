#!/bin/zsh
# Botão liga/desliga do TTS-STUDIO. Dê dois cliques no Finder:
...
if pgrep -f "uvicorn app:app" >/dev/null 2>&1; then
  ...
  nohup ./run.sh >/tmp/tts-studio.log 2>&1 &
  ...
  osascript -e 'display notification "Servidor iniciado em http://127.0.0.1:7860" with title "TTS-STUDIO"' 2>/dev/null
  echo "TTS-STUDIO iniciado. Log em /tmp/tts-studio.log"
fi
sleep 1
